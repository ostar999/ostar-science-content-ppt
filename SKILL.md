...
|------|------|------|
| `←` `→` | 切换页面 | 也支持滚轮、触屏滑动 |
| `F` | 全屏切换 | 浏览器原生全屏，隐藏工具栏和边框 |
| `N` | 总览面板 | 打开/关闭所有页面缩略图网格 |
| `ESC` | 退出全屏 | 浏览器原生行为，不会关闭总览 |

> **注意**：`ESC` 退出全屏是浏览器安全机制，JavaScript 无法拦截。为避免冲突，总览退出统一用 `N` 键，不用 `ESC`。
...
| 模式 | 触发方式 | 说明 |
|------|---------|------|
| 模式一（AI_Animation.html） | 导航栏 `⊞` 按钮 **或** 按 `N` 键 | 保留按钮的同时支持键盘快捷键 |
| 模式二（模板重构版） | 按 `N` 键 | 页面无可见按钮，纯键盘操作 |

### 总览面板特性

- 按 `N` 键打开全屏总览面板，显示所有页面 25% 缩略图卡片网格
- 每张卡片显示页码（右上角）和页面标题（底部）
- 顶部居中显示"**再次按下 N 键，退出预览**"横幅提示
- 点击任意卡片直接跳转到对应页面并退出总览
- 再次按 `N` 或点击 ✕ 按钮退出总览
- 缩略图中装饰元素（粒子、光晕）自动隐藏确保清晰
- 动画冻结在可见状态便于快速浏览
- 总览与全屏互不干扰：`N` 关总览，`ESC` 关全屏

### 实现要点

生成 HTML 时需确保：
1. 所有动画元素使用 `.an` 或 `.anim-item` class 名
2. 每个 slide 元素有唯一标识（id 或 data-index）
3. 总览面板通过 `cloneNode` 生成缩略图，冻结动画状态
4. `F` 键全屏使用浏览器 Fullscreen API（`requestFullscreen` / `exitFullscreen`）
5. 总览提示作为 flex 流式布局子元素，避免与卡片重叠
