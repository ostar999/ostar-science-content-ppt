...
生成的 HTML 必须包含以下键盘交互，参考模板见 `assets/templates/interaction-examples/`。

### 快捷键

| 按键 | 功能 | 说明 |
|------|------|------|
| `←` `→` | 切换页面 | 也支持滚轮、触屏滑动、点击左右半屏 |
| `F` | 全屏切换 | 浏览器 Fullscreen API，隐藏工具栏和边框 |
| `N` | 总览面板 | 打开/关闭所有页面 25% 缩略图网格 |
| `ESC` | 退出全屏 | 浏览器原生行为 |

### 设计原则

1. **`N` 管总览，`ESC` 管全屏** — 互不冲突。浏览器 ESC 退出全屏是安全机制无法拦截，所以总览不用 ESC
2. **模式一（原始版）** — 保留导航栏按钮（← → ⊞），同时支持键盘快捷键
3. **模式二（模板重构版）** — 页面无任何可见 UI 按钮，纯键盘操作
4. **总览提示** — 使用 flex 流式横幅（非 fixed 定位），避免与预览卡片重叠。文字："再次按下 N 键，退出预览"
5. **无自定义 toast** — 浏览器原生已提供退出全屏提示，不重复添加
6. **🚫 禁止键盘提示条** — **任何模式下，页面中不得出现键盘快捷键提示条/提示文字（如"按 F 全屏""按 N 总览""← → 切换"等）**。快捷键是隐藏功能，用户自会探索，不需要在页面上展示。总览面板内的"再次按下 N 键，退出预览"是唯一的例外。

### 总览面板实现要点

```javascript
// 1. 通过 cloneNode 生成缩略图，冻结动画
clone.querySelectorAll('.an,.anim-item').forEach(el => {
  el.style.opacity = '1';
...
// 2. 隐藏装饰元素保证清晰度
clone.querySelectorAll('.particle,.deco,.glow-orb').forEach(el => {
  el.style.display = 'none';
});

// 3. 提示作为 flex 子元素，不被卡片遮挡
...
新生成的 HTML 应参考以下模板的交互实现：

| 模板 | 路径 | 说明 |
|------|------|------|
| 原始轮播版 | `assets/templates/interaction-examples/AI_Animation.html` | 15页 · Font Awesome · 深蓝主题 · 导航栏按钮+N键 |
| Level2 重构版 | `assets/templates/interaction-examples/AI_Animation_level2.html` | 9页 · Lucide · 紫色主题 · 纯键盘 |
| 单页流程图版 | `assets/templates/interaction-examples/AI_Animation_flow.html` | 单页 · Canvas粒子 · 噪声背景 · 架构管道 |

> 生成新 HTML 时，参考上述模板的：全屏（F键）、总览面板（N键）、动画重置（cloneNode）、提示横幅（flex流式）的具体代码实现。
